...
function zoom(classNames, settings, callback) {
  ...
  massAddEventListener($zoom,"mousemove", mouseMove);

  massAddEventListener($zoom,"mouseup", mouseUp);

  massAddEventListener($zoom,"touchstart", function () {
    touchable = true;
  });

  massAddEventListener($zoom, "touchstart", touchStart);

  massAddEventListener($zoom, "touchmove", touchMove, {passive: false}); // Google Chrome - [Intervention] Unable to preventDefault inside passive event listener due to target being treated as passive.

  massAddEventListener($zoom, "touchend", touchEnd);
